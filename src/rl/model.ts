...
import { DEFAULT_HIDDEN, mlpDim, initMlp, mlpPolicy, parseMlp, type MlpWeights } from './mlp';
...
export function mlpModel(hidden: number = DEFAULT_HIDDEN): Model {
  return {
    kind: 'mlp',
    label: `mlp 30-${hidden}-1 (${mlpDim(hidden)} params)`,
    dim: mlpDim(hidden),
    init: (seed) => initMlp(hidden, seed),
    policy: (params) => mlpPolicy(params, hidden),
    file: (params, meta) => ({ kind: 'mlp-v1', hidden, weights: params, ...meta }),
  };
}

export function modelByName(kind: string, hidden: number): Model {
  if (kind === 'linear') {
    return LINEAR_MODEL;
  }
  if (kind === 'mlp') {
    return mlpModel(hidden);
  }
